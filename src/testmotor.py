...
#
#define GPIO pins
direction= 25 # Direction (DIR) GPIO Pin
...
#https://github.com/gavinlyonsrepo/RpiMotorLib.git
...
DIR_PIN  = 25 #goes to DRV-8825 DIR input (direction)
...
COUNTS_PER_FULL_STEP = 41 # 8192 / 200 # 200 steps per revolution, 8192 counts per revolution
...
pi.set_mode(STEP_PIN, pigpio.OUTPUT) #the pin that tells the stepper motor to move one step
pi.write(STEP_PIN, 0) #keep STEP low when idle so that the motor does not move on its own
...
def enc_cb(gpio, level, tick):
    ...
    if level != last_a: #any edge on A
        ...
        while (time.perf_counter_ns() - t0) < 5_000: pass
...
try:
    while True:
        ...
        if cmd == "f":
            ...
            # If the command is just a number, move that many steps
            n = int(cmd)
            fixed_steps(n)   # or move_steps(n) if you want encoder-based
        elif cmd == "gas":
            goto(TEST_CLICKS)
        elif cmd == "open":
            open_valve()
